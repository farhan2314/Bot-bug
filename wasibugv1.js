...
module.exports = XeonBotInc = async (XeonBotInc, m, msg, chatUpdate, store) => {
  try {
    ...
    //     `24/7 Online Bot By Asep Bot Owner ${ownername} autobio by NIGGA bug bot`
    //   ).catch((_) => _);
    // }
      ...
      case "grup":
        ...
        if (args[0] === "close") {
          await XeonBotInc.groupSettingUpdate(m.chat, "announcement")
            .then((res) => replygcxeon(`Success In Closing The Group ðŸ•Šï¸`))
            .catch((err) => replygcxeon(json(err)));
        } else if (args[0] === "open") {
          await XeonBotInc.groupSettingUpdate(m.chat, "not_announcement")
            .then((res) => replygcxeon(`Success In Opening The Group ðŸ•Šï¸`))
            .catch((err) => replygcxeon(json(err)));
        } else {
          replygcxeon(
      ...
      case "emojimix":
        {
          ...
          if (!emoji1)
            return replygcxeon(`Example : ${prefix + command} ðŸ˜…+ðŸ¤”`);
          if (!emoji2)
            return replygcxeon(`Example : ${prefix + command} ðŸ˜…+ðŸ¤”`);
          replygcxeon(mess.wait);
      ...
      case "song":
        {
          if (!text)
            return replygcxeon(
              `Example : ${prefix + command} Tum Hi ho ringtone`
            );
