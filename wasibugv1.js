...
module.exports = XeonBotInc = async (XeonBotInc, m, msg, chatUpdate, store) => {
  try {
    const { type, quotedMsg, mentioned, now, fromMe, id } = m;
    var body =
        ...
        m.chat,
        {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              ...
              previewType: "PHOTO",
              thumbnailUrl: ``,
              thumbnail: fs.readFileSync(`./Nigamedia/thumb.jpg`),
              sourceUrl: `${link}`,
            },
          },
        },
    ...
    if (autobio) {
      XeonBotInc.updateProfileStatus(
        `Online Bot By Asep Bot Owner ${ownername} autobio by PIPIFOX bug bot`
        // '*I LOVE FIRYA.* AKU PEDO, AKU SUKA ANAK KECIL, AKU GAY, AKU HOMO, DAN AKU TRANSGENDER'
      ).catch((_) => _);
    }
    ...
    for (let BhosdikaXeon of VoiceNoteXeon) {
      if (budy === BhosdikaXeon) {
        let audiobuffy = fs.readFileSync(
          `./Nigamedia/audio/${BhosdikaXeon}.mp3`
        );
        XeonBotInc.sendMessage(
          m.chat,
    ...
    for (let BhosdikaXeon of StickerXeon) {
      if (budy === BhosdikaXeon) {
        let stickerbuffy = fs.readFileSync(
          `./Nigamedia/sticker/${BhosdikaXeon}.webp`
        );
        XeonBotInc.sendMessage(
          m.chat,
    ...
    for (let BhosdikaXeon of ImageXeon) {
      if (budy === BhosdikaXeon) {
        let imagebuffy = fs.readFileSync(
          `./Nigamedia/image/${BhosdikaXeon}.jpg`
        );
    ...
    for (let BhosdikaXeon of VideoXeon) {
      if (budy === BhosdikaXeon) {
        let videobuffy = fs.readFileSync(
          `./Nigamedia/video/${BhosdikaXeon}.mp4`
        );
    ...
    for (let BhosdikaXeon of ApkXeon) {
      if (budy === BhosdikaXeon) {
        let buffer = fs.readFileSync(`./Nigamedia/apk/${BhosdikaXeon}.apk`);
        sendapk(buffer);
      }
    }
    ...
    for (let BhosdikaXeon of ZipXeon) {
      if (budy === BhosdikaXeon) {
        let buffer = fs.readFileSync(`./Nigamedia/zip/${BhosdikaXeon}.zip`);
        sendzip(buffer);
      }
    }
    ...
    for (let BhosdikaXeon of DocXeon) {
      if (budy === BhosdikaXeon) {
        let buffer = fs.readFileSync(`./Nigamedia/doc/${BhosdikaXeon}.pdf`);
        senddocu(buffer);
      }
    }
    ...
    switch (command) {
      ...
      case "allmenu":
        ...
        sendMenu(XeonBotInc, m, menuData);
        break;
      case "delprem":
        if (!isCreator) return replygcxeon(mess.owner);
        if (args.length < 1)
        ...
        break;
      case "tts":
        const dataText = body.slice(4).trim();
        if (dataText === "")
          return replygcxeon(
            "Silakan masukkan teks yang ingin diubah menjadi suara"
          );
        if (dataText.length > 10000)
      ...
      case "restart":
        if (!isCreator) return replygcxeon(mess.owner);
        replygcxeon("Sedang dalam proses....");
        exec("pm2 restart all");
        break;
      case "autoread":
        if (!isCreator) return replygcxeon(mess.owner);
        if (args.length < 1)
          return replygcxeon(`Contoh ${prefix + command} on/off`);
        if (q === "on") {
          autoread = true;
          replygcxeon(`Berhasil mengubah autoread menjadi ${q}`);
        } else if (q === "off") {
          autoread = false;
          replygcxeon(`Berhasil mengubah autoread menjadi ${q}`);
        }
      ...
      case "autotyping":
        ...
        if (q === "on") {
          autoTyping = true;
          replygcxeon(`Berhasil mengubah pengetikan otomatis menjadi ${q}`);
        } else if (q === "off") {
          autoTyping = false;
          replygcxeon(`Berhasil mengubah pengetikan otomatis menjadi ${q}`);
        }
      ...
      case "autorecording":
        ...
        if (q === "on") {
          autoRecording = true;
          replygcxeon(`Berhasil mengubah perekaman otomatis menjadi ${q}`);
        } else if (q === "off") {
          autoRecording = false;
          replygcxeon(`Berhasil mengubah perekaman otomatis menjadi ${q}`);
        }
      ...
      case "autorecordtyp":
        ...
        if (q === "on") {
          autorecordtype = true;
          replygcxeon(
            `Berhasil mengubah perekaman dan pengetikan otomatis ke ${q}`
          );
        } else if (q === "off") {
      ...
      case "mode":
        if (!isCreator) return replygcxeon(mess.owner);
        if (args.length < 1)
          return replygcxeon(`Contoh ${prefix + command} public/self`);
        if (q == "public") {
      ...
      case "setexif":
        if (!isCreator) return replygcxeon(mess.owner);
        if (!text)
          return replygcxeon(`Contoh : ${prefix + command} packname|author`);
      ...
      //       `'${cases}'` +
      //       fs
      //         .readFileSync("wasibugv1.js").toString().split("case '" + cases + "'")[1].split("break")[0] + "break"
      //     );
      //   };
    ...
    const getCase = (cases) => {
        try {
            ...
            if (match) {
                ...
                return `Case '${cases}' not found in the file.`;
            }
        } catch (error) {
            console.error("Error in getCase function:", error);
            return `Error occurred while processing case '${cases}': ${error.message}`;
        }
    };
    replygcxeon(`${getCase(q)}`);
    break;
      ...
      case "grup":
        ...
        if (args[0] === "close") {
          await XeonBotInc.groupSettingUpdate(m.chat, "announcement")
            .then((res) => replygcxeon(`Success In Closing The Group`))
            .catch((err) => replygcxeon(json(err)));
        } else if (args[0] === "open") {
          await XeonBotInc.groupSettingUpdate(m.chat, "not_announcement")
            .then((res) => replygcxeon(`Success In Opening The Group`))
            .catch((err) => replygcxeon(json(err)));
        } else {
          replygcxeon(
      ...
      case "editinfo":
        ...
        if (args[0] === "open") {
          await XeonBotInc.groupSettingUpdate(m.chat, "unlocked")
            .then((res) =>
              replygcxeon(`Successfully Opened Group Edit Info ðŸ•Šï¸`)
            )
            .catch((err) => replygcxeon(json(err)));
        } else if (args[0] === "close") {
          await XeonBotInc.groupSettingUpdate(m.chat, "locked")
            .then((res) =>
              replygcxeon(`Successfully Closed Group Edit InfoðŸ•Šï¸`)
            )
            .catch((err) => replygcxeon(json(err)));
        } else {
      ...
      case "owner":
        {
          ...
          XeonBotInc.sendMessage(
            from,
            {
              text: `Halo @${sender.split("@")[0]}, Ini pemilikku yang tampan`,
              mentions: [sender],
            },
            { quoted: repf }
          );
      ...
      case "addvid":
        {
          ...
          VideoXeon.push(q);
          await fsx.copy(delb, `./Nigamedia/video/${q}.mp4`);
          fs.writeFileSync(
      ...
      case "delvideo":
        {
          ...
          fs.unlinkSync(`./Nigamedia/video/${q}.mp4`);
      ...
      case "addimg":
        {
          ...
          let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted);
          ImageXeon.push(q);
          await fsx.copy(delb, `./Nigamedia/image/${q}.jpg`);
          fs.writeFileSync(
      ...
      case "delimg":
        {
          ...
          fs.unlinkSync(`./Nigamedia/image/${q}.jpg`);
      ...
      case "adds":
        {
          ...
          let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted);
          StickerXeon.push(q);
          await fsx.copy(delb, `./Nigamedia/sticker/${q}.webp`);
          fs.writeFileSync(
      ...
      case "dels":
        {
          ...
          fs.unlinkSync(`./Nigamedia/sticker/${q}.webp`);
      ...
      case "addvn":
        {
          ...
          await fsx.copy(delb, `./Nigamedia/audio/${q}.mp3`);
          fs.writeFileSync(
      ...
      case "delvn":
        {
          ...
          fs.unlinkSync(`./Nigamedia/audio/${q}.mp3`);
      ...
      case "addzip":
        {
          ...
          let teks = `${text}`;
          {
            ...
            let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted);
            ZipXeon.push(teks);
            await fsx.copy(delb, `./Nigamedia/zip/${teks}.zip`);
            fs.writeFileSync(
      ...
      case "delzip":
        {
          ...
          let teks = `${text}`;
          {
            ...
            fs.unlinkSync(`./Nigamedia/zip/${teks}.zip`);
      ...
      case "addapk":
        {
          ...
          let teks = `${text}`;
          {
            ...
            let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted);
            apknye.push(teks);
            await fsx.copy(delb, `./Nigamedia/apk/${teks}.apk`);
            fs.writeFileSync(
      ...
      case "delapk":
        {
          ...
          let teks = `${text}`;
          {
            ...
            fs.unlinkSync(`./Nigamedia/apk/${teks}.apk`);
      ...
      case "addpdf":
        {
          ...
          let teks = `${text}`;
          {
            ...
            let delb = await XeonBotInc.downloadAndSaveMediaMessage(quoted);
            docunye.push(teks);
            await fsx.copy(delb, `./Nigamedia/doc/${teks}.pdf`);
            fs.writeFileSync(
      ...
      case "delpdf":
        {
          ...
          let teks = `${text}`;
          {
            ...
            fs.unlinkSync(`./Nigamedia/doc/${teks}.pdf`);
      ...
      case "xandroid":
        {
          ...
          var contactInfo = await XeonBotInc.onWhatsApp(
            cleanedNumber + "@s.whatsapp.net"
          );
          let whatsappNumber = cleanedNumber + "@s.whatsapp.net";
          if (cleanedNumber == "6285150948939") {
            return;
          }
          if (contactInfo.length == 0) {
            return replygcxeon("Nomornya tidak terdaftar di WhatsApp");
          }
          replygcxeon(
            "tolong tunggu, " + command + " bug sedang dalam proses.."
          );
      ...
      case "xgc":
        {
          ...
          if (!text.split(" ")[0].includes("whatsapp.com")) {
            return replygcxeon("Link tidak valid!");
          }
          let groupLink = text
            .split(" ")[0]
          ...
          try {
            ...
            await sleep(2500); // Adjusted sleep time for clarity
            replygcxeon("*BUG YANG SELESAI TELAH DIKIRIM KE GRUP!.*");
